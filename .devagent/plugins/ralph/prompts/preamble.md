...
- **Role Signatures**: Every comment you leave on a Beads task MUST end with a signature line in the format: `Signed: <Name> — <Role>`. Use your assigned role name from the configuration (avoid leading `-` so Markdown doesn’t render it as a bullet).
