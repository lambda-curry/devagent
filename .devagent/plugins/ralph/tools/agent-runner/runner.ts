 ...
 * runner.ts
 *
 * Core logic for running agents with retries, logging, and wake notifications.
 */
import { spawn } from 'node:child_process';
import { resolve } from 'node:path';
import { AgentRunOptions, AgentRunResult, AgentDefinition } from './types';
import { registry } from './registry';
import { nowStamp, setupLogDir, writeLogHeader, writeLog } from './utils/logging';
import { sleep, calculateNextDelay } from './utils/retry';
import { maybeWake, buildWakeMessage } from './utils/wake';
...
export async function runAgent(opts: AgentRunOptions): Promise<AgentRunResult> {
  // Validate agent
  const config = registry.get(opts.agent);
  if (!config) {
    throw new Error(`Unknown agent: ${opts.agent}. Supported agents: ${registry.list().join(', ')}`);
  }
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    try {
      ...
      if (result.exitCode === 0) {
        ...
        let wakeResult = { sent: false, error: undefined };
