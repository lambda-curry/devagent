...
- `.devagent/workspace/features/{status}/YYYY-MM-DD_feature-slug/plan/` — canonical location for active plans and change history. (Note: replacing previous `spec/` and `tasks/` separation.)
...
## Knowledge Sources
- Internal: Product constitution, existing feature plans, ADRs, analytics dashboards, customer feedback archives, prior research packets.
...
9. **Output packaging:** Save the plan to `.devagent/workspace/features/{status}/YYYY-MM-DD_feature-slug/plan/YYYY-MM-DD_<descriptor>.md`, update the feature hub summary, and communicate key product decisions plus implementation strategy to the requester.
...
## Expected Output
- Artifacts: Markdown plan using the unified plan document template, stored under the feature's `plan/` directory with ISO date prefix, plus any updates to feature README or index files when needed.
...
- Communication: Surface plan to reviewing stakeholders (product, design, engineering leads) per team conventions; record approval and any waivers in feature hub.
