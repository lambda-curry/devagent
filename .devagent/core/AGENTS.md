...
- `devagent new-workspace` — Creates a new git worktree (workspace) and optionally migrates uncommitted work from the current workspace to the new workspace, enabling concurrent feature development without context switching or stashing conflicts. Utilize when you need to work on multiple features simultaneously or spin off unrelated work into a separate workspace. See `.devagent/core/workflows/new-workspace.md`.
...
| new-workspace | Creates a new git worktree and optionally migrates uncommitted work |
