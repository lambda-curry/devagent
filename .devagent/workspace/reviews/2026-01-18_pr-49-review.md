...
- Linear Issues: ⚠️ No Linear issues linked
...
- **Critical comments:** 3 (should address before merge)
  ...
  - `.devagent/plugins/ralph/agents/implementation-agent.json`: rename to “Engineering Agent” requires label/routing/doc consistency (switch routing label to `engineering`)
- **Minor comments:** 4 (nice to have)
  - `.devagent/plugins/ralph/agents/implementation-agent-instructions.md`: heading says “Implementation Agent Instructions” but agent name is “Engineering Agent”
...
- `ralph.ts` now maps Beads CLI comment `text` → `body`, and `BeadsTaskDetails` no longer uses an unsafe `extends`-with-retyped-fields pattern.
...
**Critical Issues:**
- **Non-zero exit propagation missing** (`.devagent/plugins/ralph/tools/ralph.sh`): after running the router, the script detects `$EXIT_CODE -ne 0` but only logs an error and continues. This can mask router failures for automation. Recommendation: exit with the router’s exit code immediately after logging.

**Major Issues:**
- **Potential hung process** (`apps/ralph-monitoring/app/db/beads.server.ts`): `execBdCommentsJson()` calls `execFile('bd', ...)` without a timeout. If `bd` stalls, comment-count retrieval can hang indefinitely. Recommendation: pass `timeout` in the `execFile` options, or reuse `getTaskCommentsAsync()`’s timeout approach.
...
- **Agent rename consistency** (`.devagent/plugins/ralph/agents/implementation-agent.json` + docs/config/routing): PR changes the agent “name” to “Engineering Agent”. Ensure routing uses the `engineering` label consistently across config + docs so label-driven routing remains correct.

**Minor Issues:**
- **Docs mismatch** (`.devagent/plugins/ralph/agents/implementation-agent-instructions.md`): heading should match configured agent name.
- **Docs placeholder bug** (`.devagent/plugins/ralph/skills/plan-to-beads-conversion/SKILL.md`): “blocked” epic update command still uses `bd-<hash>` instead of `<epic-id>`.
...
- Establishes clearer Beads data “trust boundary” (normalization + parsing context).
...
- Naming/routing consistency around “Engineering Agent” and the `engineering` label should be enforced to prevent future misroutes.
...
**Confidence Factors:**
- ✅ Positive indicators: strong feature set, improved resiliency around comments, solid testing investment, clear docs.
- ⚠️ Concerns affecting confidence: open review items include failure masking + potential hung processes.
- 🔴 Blockers: `ralph.sh` should exit non-zero on router failure.
...
- [ ] Make the “Engineering Agent” rename consistent across docs/config/routing (use `engineering` as the routing label).
- [ ] Update headings/placeholders in docs (`implementation-agent-instructions.md`, `plan-to-beads-conversion/SKILL.md`).
...
- This PR is very large (58 files, ~3.3k additions). If you want faster re-review cycles, consider splitting follow-up fixes (exit-code + timeout + typing) into a small “merge-blockers” PR on top of this branch.
