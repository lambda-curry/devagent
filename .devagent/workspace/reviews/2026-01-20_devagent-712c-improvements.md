...
### Documentation
- [ ] **[High] Make revise-report invocation executable**: `devagent ralph-revise-report <epic-id>` is documented as a shell command, but `devagent` is not a CLI binary in this environment; either provide a runnable entrypoint or explicitly document the manual generation fallback. **Source**: devagent-712c.6
...
## Action Items
1. [ ] **[High | Documentation]** Provide a runnable revise-report generation entrypoint (or document manual fallback) so `devagent ralph-revise-report <epic-id>` doesn’t fail in shell contexts. (devagent-712c.6)
