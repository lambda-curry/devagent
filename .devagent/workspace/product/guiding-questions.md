...
| 2026-01-19 | What is DevAgent’s recommended Beads “low-noise” setup for new repos (sync-branch vs DB-only relocation via `BEADS_DB` vs true sister-repo via `BEADS_DIR`)? | Internal docs cover sync-branch and DB-path override; `BEADS_DIR` semantics and multi-repo flow are not yet evidenced in `.devagent/**`. | TBD - validate Beads primary docs and run a small experiment; then document a default + decision tree. |
