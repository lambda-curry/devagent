...
### Scope Definition
- **In Scope:**
  - Update 8 workflows with explicit date retrieval instructions (research, create-plan, new-feature, review-progress, brainstorm, compare-prs, review-pr, clarify-task)
...
**Trigger:** AI assistant invokes workflow that creates dated document (research, plan, task hub, checkpoint, brainstorm packet, review artifact, etc.)
...
- **Subtasks:**
  ...
  2. Update `create-plan.md` — Add date retrieval step in "Output packaging" section before creating plan documents
     - Validation: Review workflow for explicit `date +%Y-%m-%d` instruction
  3. Update `new-feature.md` — Add date retrieval step in "Kickoff" section before determining task_prefix and in "AGENTS.md population" section
     - Validation: Review workflow for explicit `date +%Y-%m-%d` instruction
...
#### Task 3: Update Ideation and Clarification Workflows (brainstorm, clarify-task)
...
- **Impacted Modules/Files:**
  ...
  - `.devagent/core/workflows/clarify-task.md`
- **Dependencies:** None
- **Acceptance Criteria:**
  - Each workflow includes explicit step: "Get current date: Run `date +%Y-%m-%d`"
...
- **Subtasks:**
  1. Update `brainstorm.md` — Add date retrieval step in "Package outputs" section before creating brainstorm packet
     - Validation: Review workflow for explicit `date +%Y-%m-%d` instruction
  2. Update `clarify-task.md` — Verify date usage and add date retrieval step if clarification packets are dated
...
- **Subtasks:**
  1. Update `task-agents-template.md` — Update "Agent Update Instructions" section to specify `date +%Y-%m-%d` for "Last Updated" field
...
- **Impacted Modules/Files:**
  ...
  - `.devagent/workspace/tasks/completed/2025-12-27_consistent-date-handling-workflows/AGENTS.md` (update completion status)
...
## Progress Tracking
Refer to the AGENTS.md file in the task directory (`.devagent/workspace/tasks/completed/2025-12-27_consistent-date-handling-workflows/AGENTS.md`) for instructions on tracking and reporting progress during implementation.
...
- `.devagent/core/workflows/clarify-task.md` — Will use updated date handling patterns

### Templates
- `.devagent/core/templates/task-agents-template.md` — Will include explicit date retrieval instructions
