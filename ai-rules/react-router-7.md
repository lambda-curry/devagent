---
description: Enforces correct import patterns and package usage for React Router v7 framework mode, ensuring type safety and preventing common mistakes with route type imports.
fileMatching: "**/*"
alwaysApply: true
---
...
Types are autogenerated and should be imported as `./+types/[routeFileName]`. **If you're getting a type error, run `bun run typecheck` first.**
...
- **Auto-generated**: Created by `@react-router/dev` when you run dev server or `bun run typecheck`
...
### RR7 Response Helpers and Headers

- **Use `data()` for JSON responses**. `json()` is deprecated in RR7 framework mode.
- **Never pass a `Headers` instance** to `redirect()` or `data()`; pass a plain object (`Record<string,string>`) or tuple list instead.
- When aggregating `Set-Cookie` or composing headers across calls, **normalize** any `HeadersInit` to a plain object before returning.

```ts
// Helper function for header normalization
export function headersToObject(input?: HeadersInit): Record<string, string> | undefined {
  if (!input) return undefined;
  const out: Record<string, string> = {};
  new Headers(input).forEach((v, k) => (out[k] = v));
  return Object.keys(out).length ? out : undefined;
}

// Usage in an action/loader
const headers = headersToObject(responseHeaders);
return redirect('/chat', headers ? { headers } : undefined);
// For JSON payloads
return data({ ok: true }, headers ? { headers } : undefined);
```
...
### Bouncing between sibling routes (Settings ↔ Chat)

- For persistent controls like "Back to Chat" that live outside complex layouts, use **router primitives** with type-safe URLs and prefetching:

```tsx
import { Link, href } from 'react-router';

<Link to={href('/chat')} prefetch="intent" replace>
  ← Back to Chat
</Link>
```

- Place navigational controls outside portals/overlays that manage their own focus/DOM lifecycles to avoid interference with RR7 navigation.
...
2. **RUN `bun run typecheck`** when you see missing type errors - never try to "fix" the import path
...
- **If you see missing `./+types/[routeName]` imports, ALWAYS suggest running `bun run typecheck` first**
- **NEVER suggest changing `./+types/[routeName]` to `../+types/[routeName]` or any other relative path**
- **After creating new routes, remind the user to run `bun run typecheck`**
...
- `renderRouteComponent(Component, { path, loader, action, initialEntries })`: Minimal single-route render helper.

Patterns:
- Component-level: use `createRoutesStub` so `Link`/`useHref` render under a Router.
- Route-level/integration (redirects, `useFetcher`): use the stub with multiple route objects. Assert on the DOM instead of router internals.
...
Example (route-level with API):
```tsx
const routes = [
  createRoutesFor(IngestPage, { path: '/' }),
  { path: '/api/ingest', action: async () => ({ preview: {...}, summaries: [...] }) }
];
...
Notes:
- Prefer `createRoutesStub` for most tests; avoid `<MemoryRouter>` and manual router management.
- Do not mock router hooks; assert DOM and `router.state` when needed.
